        ...
        function(resp) {
            ...
            if (resp.JoinedStatus && resp.JoinedStatus == 'JoinedSuccess') {
                ...
                selToID = groupId;
                  ...
                  alert('进群失败');
                  webim.Log.info("进群失败"+JSON.stringfy(resp) );
            }
        },
        function(err) {
            webim.Log.info("进群失败"+err.ErrorInfo);
...
//显示消息（群普通+点赞+提示+红包）
// "{"CMD":"RecommandProduct","product":"你好这是一条推荐产品XXX,http:url"}";产品推荐
...
nickNameSpan.removeAttribute("hidden");
    switch (subType) {
        ...
        case webim.GROUP_MSG_SUB_TYPE.TIP: //群提示消息
            contentSpan.innerHTML = "[青牛弹幕提示您]" + convertMsgtoHtml(msg);
            nickNameSpan.setAttribute("hidden",true);
            break;
        default:
...
//自定义消息通道   发题目
function onSendQuestion(data_msg, desc, ext) {
    ...
    var text_obj, face_obj, tmsg, emotionIndex, emotion, restMsgIndex;
    if (!emotions || emotions.length < 1) {
        text_obj = new webim.Msg.Elem.Custom(msgtosend, desc, ext);
...
function independentLogin(userid,usersig) {

    loginInfo.identifier =  userid;
    loginInfo.userSig = usersig;
    $('#login_dialog').hide();
    console.log('independentLogin-sdkLogin userid='+userid+"usersig="+usersig);
    sdkLogin();
}
